            ...
            lines.forEach(function(line) {{
                ...
                if (rm) {{
                    ...
                    if (n1 && n2 && n1 !== n2) {{
                        ...
                        return; // skip schedule detection for this line
                    }}
                }}

                // 2) Modification horaire : ligne contenant un nom de staff + horaire
                var foundStaff = null;
                var words = lower.replace(/[^a-z\u00e0-\u00ff\\s]/g, ' ').split(/\\s+/);
                for (var w = 0; w < words.length; w++) {{
                    var s = _findStaff(words[w]);
                    if (s) {{ foundStaff = s; break; }}
                }}
                if (foundStaff) {{
                    var times2 = _detectTimes(lower);
                    var dayIdx2 = _detectDay(lower);
                    var code = _detectCode(lower);
                    if (times2 || code) {{
                        actions.push({{
                            type: 'schedule',
                            staff: foundStaff,
                            dayIdx: dayIdx2,
                            start: times2 ? times2[0] : null,
                            end: times2 ? times2[1] : null,
                            code: code || 'VDC',
                            raw: line.trim()
                        }});
                    }}
                        ...
                        if (selectedActions && selectedActions.length > 0) {{
                            ...
                            selectedActions.forEach(function(a) {{
                                ...
                                if (a.type === 'replacement') {{
                                    ...
                                    // Modifier/ajouter un créneau dans DATA
                                    var staff = a.staff;
                                    if (DATA[staff]) {{
