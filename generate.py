...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        /* ── Bouton flottant « Programme TV » (bas à droite) ── */
        .tv-fab {{
            position: fixed; right: 16px; bottom: 16px; z-index: 90;
            display: inline-flex; align-items: center; gap: 8px;
            padding: 12px 18px; border-radius: 40px;
            background: #FF6600; color: #1A1A1A; text-decoration: none;
            font-family: 'Montserrat', sans-serif; font-size: 13px; font-weight: 800;
            text-transform: uppercase; letter-spacing: 0.5px;
            box-shadow: 0 4px 18px rgba(255,102,0,0.45);
            transition: all 0.2s;
        }}
        .tv-fab:hover {{ background: #ff8a3d; transform: translateY(-2px);
                          box-shadow: 0 6px 22px rgba(255,102,0,0.6); }}
        .tv-fab .tv-fab-ico {{ font-size: 17px; }}
        @media (max-width: 600px) {{
            .tv-fab {{ padding: 11px 15px; font-size: 12px; right: 12px; bottom: 12px; }}
        }}
        ...
        .staff-filter {{ display: flex; gap: 6px; align-items: center; margin: 6px 0 10px; }}
        .staff-filter input {{ flex: 1; padding: 8px 12px; background: rgba(255,255,255,0.05);
                               border: 1px solid rgba(255,255,255,0.1); border-radius: 6px;
                               color: #fff; font-size: 13px; font-family: inherit; outline: none; }}
        .staff-filter input:focus {{ border-color: rgba(255,102,0,0.5); }}
        .staff-filter button {{ background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1);
                                border-radius: 6px; color: #ccc; padding: 6px 12px; font-size: 14px;
                                cursor: pointer; font-family: inherit; }}
        .staff-filter button:hover {{ background: rgba(255,102,0,0.15); color: #fff; }}
        .timeline-row.hidden-by-filter {{ display: none; }}
        @media print {{
            body {{ background: #fff !important; color: #000 !important; }}
            .top-bar, .fb-warning, .week-selector, .view-toggle, .day-tabs,
            .staff-filter, .no-print, #admin-toolbar, .save-edits-btn, .tv-fab,
            .modal-overlay, .cal-chooser-overlay, .meta-note {{ display: none !important; }}
            .container {{ max-width: none !important; padding: 0 !important; }}
            .container::after {{ display: none !important; }}
            .header h1, .header .subtitle {{ color: #000 !important; }}
            .header .dates {{ color: #444 !important; }}
            .timeline {{ background: #fff !important; }}
            .tl-name {{ color: #000 !important; font-weight: 600; }}
            .tl-bar {{ box-shadow: none !important; filter: none !important;
                       print-color-adjust: exact; -webkit-print-color-adjust: exact; }}
            .tl-grid-line {{ border-color: rgba(0,0,0,0.15) !important; }}
            .time-marker {{ color: #333 !important; }}
            .legend-item {{ color: #000 !important; }}
            /* Le body normal a font-size 14px, on garde une taille lisible en print */
...
<body>
    <div class="container">
        ...
        <a href="tv.html" class="tv-fab" title="Programme TV du centre"><span class="tv-fab-ico">📺</span> Programme TV</a>
        <div class="header">
...
{week_tabs.rstrip()}
        ...
        <div id="view-day">
            ...
            <div class="staff-filter no-print">
                <input type="search" id="staff-filter-input" placeholder="🔍 Filtrer par staff…" autocomplete="off" />
                <button type="button" id="staff-filter-clear" title="Effacer">×</button>
                <button type="button" id="print-btn" title="Imprimer / PDF">🖨️</button>
            </div>
...
{employee_buttons.rstrip()}
    ...
    (function() {{
        ...
        (function() {{
            ...
            // Après 20h, on prépare probablement le lendemain : sauter à J+1
            // si celui-ci est dans la même semaine (idx+1 valide).
            if (idx !== -1 && now.getHours() >= 20 && idx + 1 < WEEK_DATES.length) {{
                currentDay = idx + 1;
            }}
        }})();
        var currentView = 'day';
        ...
        setTimeout(function() {{
            var activeTab = dayTabsEl.querySelectorAll('.day-tab.active')[0];
            if (activeTab) activeTab.scrollIntoView({{ inline: 'center', block: 'nearest' }});
        }}, 0);

        // ── Filtre staff : cache les lignes dont le nom ne match pas ──
        (function() {{
            var input = document.getElementById('staff-filter-input');
            var clearBtn = document.getElementById('staff-filter-clear');
            ...
            function applyFilter() {{
                ...
                rows.forEach(function(row) {{
                    ...
                    if (!nameEl) return; // ligne d'en-tête (time markers)
                    var full = nameEl.getAttribute('title') || '';
                    var disp = nameEl.textContent || '';
                    var match = !q || normalize(full).indexOf(q) !== -1 || normalize(disp).indexOf(q) !== -1;
            ...
            // Re-appliquer le filtre à chaque re-render de la timeline
            var origRender = window.renderTimeline;
            window.__reapplyStaffFilter = applyFilter;
        }})();
        ...
        function renderTimeline() {{
            ...
            // Ré-appliquer le filtre staff après chaque re-render
            if (typeof window.__reapplyStaffFilter === 'function') window.__reapplyStaffFilter();
        ...
        function openModal(name) {{
            ...
            if (!hasDays) {{
                ...
                var totalH = computeWeeklyHours(emp, name);
                var footer = document.createElement('div');
                footer.className = 'modal-hours-total';
                var lines = [];
                if (totalH.replRemoved > 0 || totalH.replAdded > 0) {{
                    lines.push('<div class="hours-line">Cr\u00e9neaux propres : <strong>' + formatHours(totalH.brutOwn) + '</strong></div>');
                    if (totalH.replAdded > 0) lines.push('<div class="hours-line" style="color:#51cf66;">Rempla\u00e7ant : <strong>+' + formatHours(totalH.replAdded) + '</strong></div>');
                    if (totalH.replRemoved > 0) lines.push('<div class="hours-line" style="color:#ff6b6b;">Remplac\u00e9 : <strong>\u2212' + formatHours(totalH.replRemoved) + '</strong></div>');
                    lines.push('<div class="hours-line">Heures brut effectives : <strong>' + formatHours(totalH.brut) + '</strong></div>');
                }} else {{
                    lines.push('<div class="hours-line">Heures brut : <strong>' + formatHours(totalH.brut) + '</strong></div>');
                }}
                if (totalH.pause > 0) {{
                    lines.push('<div class="hours-line pause">Pauses (20min / 6h) : <strong>\u2212' + formatHours(totalH.pause) + '</strong></div>');
                    lines.push('<div class="hours-line net">Heures net : <strong>' + formatHours(totalH.net) + '</strong></div>');
                }} else if (!(totalH.replRemoved > 0 || totalH.replAdded > 0)) {{
                    // Cas simple : ni pause ni remplacement \u2014 on avait juste un total
                    lines = ['Total semaine : <strong>' + formatHours(totalH.brut) + '</strong>'];
                }}
        ...
        // Retourne l'ajustement d'heures dû aux remplacements pour un staff :
        //   removed = somme des heures où il est OUT (barre .replaced) — ces
        //             heures sont dans ses events mais quelqu'un d'autre les fait
        //   added   = somme des heures où il est IN (remplace qqun) et qu'il n'a
        //             pas déjà de créneau propre couvrant cette plage
        function computeReplacementDelta(emp, name) {{
            var removed = 0, added = 0;
            if (!name) return {{ removed: 0, added: 0 }};
            var repls = getReplacements();
            for (var i = 0; i < repls.length; i++) {{
                var r = repls[i];
                var dateStr = r.date;
                if (!dateStr) continue;
                var rs = new Date(dateStr + 'T' + r.start).getTime();
                var re = new Date(dateStr + 'T' + r.end).getTime();
                if (re <= rs) re += 24 * 3600 * 1000; // passage minuit
                if (name === r.out) {{
                    // Retire l'intersection avec ses propres events
                    emp.events.forEach(function(ev) {{
                        var es = new Date(ev.start).getTime();
                        var ee = new Date(ev.end).getTime();
                        var interS = Math.max(es, rs), interE = Math.min(ee, re);
                        if (interE > interS) removed += (interE - interS) / 3600000;
                    }});
                }} else if (name === r['in']) {{
                    // Ajoute la fenêtre du remplacement moins ce qu'il fait déjà
                    var covered = 0;
                    emp.events.forEach(function(ev) {{
                        var es = new Date(ev.start).getTime();
                        var ee = new Date(ev.end).getTime();
                        var interS = Math.max(es, rs), interE = Math.min(ee, re);
                        if (interE > interS) covered += (interE - interS);
                    }});
                    var winMs = re - rs;
                    var extra = (winMs - covered) / 3600000;
        ...
        function computeWeeklyHours(emp, name) {{
            var brutOwn = 0;
            emp.events.forEach(function(ev) {{
                var s = new Date(ev.start);
                var e = new Date(ev.end);
                brutOwn += (e - s) / (1000 * 60 * 60);
            }});
            var delta = computeReplacementDelta(emp, name);
            var brut = brutOwn - delta.removed + delta.added;
            var pauseH = computeWeeklyPause(emp);
            return {{
                brut: brut, net: brut - pauseH, pause: pauseH,
                brutOwn: brutOwn, replRemoved: delta.removed, replAdded: delta.added
            }};
        }}
        ...
        function updateHoursBadges() {{
            document.querySelectorAll('.employee-btn[data-name]').forEach(function(btn) {{
                ...
                var h = computeWeeklyHours(emp, name);
            ...
            selectedActions.forEach(function(a) {{
                ...
                if (a.type === 'replacement') {{
                    ...
                    if (DATA[staff]) {{
                        ...
                        // existing events on the same day. Trim/split/remove conflicting events.
                        var kept = [];
                        DATA[staff].events.forEach(function(ev) {{
                            if (ev.day !== dayIdx) {{ kept.push(ev); return; }}
                            var evStartT = new Date(ev.start).getTime();
                            var evEndT = new Date(ev.end).getTime();
                            // No overlap
                            if (evEndT <= newStartT || evStartT >= newEndT) {{ kept.push(ev); return; }}
                            // Existing fully inside new → drop
                            if (evStartT >= newStartT && evEndT <= newEndT) return;
                            // Existing surrounds new → split into before + after
                            if (evStartT < newStartT && evEndT > newEndT) {{
                                kept.push(Object.assign({{}}, ev, {{ end: startISO }}));
                                kept.push(Object.assign({{}}, ev, {{ start: endISO }}));
                                return;
                            }}
                            // Existing starts before, ends inside → trim end
                            if (evStartT < newStartT) {{
                                kept.push(Object.assign({{}}, ev, {{ end: startISO }}));
                                return;
                            }}
                            // Existing starts inside, ends after → trim start
                            kept.push(Object.assign({{}}, ev, {{ start: endISO }}));
                        }});
                        kept.push({{
                            code: a.code || 'VDC',
        ...
        // ── RH / cohérence : détection de chevauchements et dépassement 48h ──
        function checkOverlap(name, dateStr, startISO, endISO, ignoreEv) {{
            var emp = DATA[name];
            if (!emp) return [];
            var ns = new Date(startISO).getTime();
            var ne = new Date(endISO).getTime();
            var out = [];
            emp.events.forEach(function(ev) {{
                if (ev === ignoreEv) return;
                if (ev.start.substring(0, 10) !== dateStr.substring(0, 10)) return;
                var es = new Date(ev.start).getTime();
                var ee = new Date(ev.end).getTime();
                if (es < ne && ee > ns) {{
                    out.push({{ start: ev.start.substring(11,16), end: ev.end.substring(11,16), code: ev.code }});
                }}
            }});
            return out;
        }}

        // Vérifie les fenêtres 7 jours glissants qui incluraient un nouveau
        // créneau, en tenant compte des heures propres + effet des remplacements.
        function check48hRolling(name, extraStartISO, extraEndISO) {{
            var emp = DATA[name];
            if (!emp) return {{ worstMargin: null }};
            var extraHours = (new Date(extraEndISO).getTime() - new Date(extraStartISO).getTime()) / 3600000;
            var extraDate = extraStartISO.substring(0, 10);
            var target = new Date(extraDate + 'T00:00');
            var worst = 999;
            var worstWin = null;
            for (var off = 0; off < 7; off++) {{
                var winStart = new Date(target); winStart.setDate(winStart.getDate() - (6 - off));
                var winEnd = new Date(winStart); winEnd.setDate(winEnd.getDate() + 7);
                var total = 0;
                emp.events.forEach(function(ev) {{
                    var es = new Date(ev.start).getTime();
                    var ee = new Date(ev.end).getTime();
                    var interS = Math.max(es, winStart.getTime()), interE = Math.min(ee, winEnd.getTime());
                    if (interE > interS) total += (interE - interS) / 3600000;
                }});
                total += extraHours; // ajoute le nouveau créneau
                var margin = 48 - total;
                if (margin < worst) {{ worst = margin; worstWin = {{ start: winStart, end: winEnd, hours: total }}; }}
            }}
            return {{ worstMargin: worst, worstWindow: worstWin }};
        }}

        // Petit dialogue "Continuer / Annuler" avec message HTML libre.
        function showEditWarning(htmlMsg, onProceed, onCancel) {{
            var overlay = document.createElement('div');
            overlay.className = 'confirm-overlay';
            overlay.innerHTML =
                '<div class="confirm-dialog">' +
                    '<div class="confirm-icon">⚠️</div>' +
                    '<div class="confirm-msg">' + htmlMsg + '</div>' +
                    '<div class="confirm-actions">' +
                        '<button class="btn-confirm-save">Continuer quand même</button>' +
                        '<button class="btn-confirm-cancel">Annuler</button>' +
                    '</div>' +
                '</div>';
            document.body.appendChild(overlay);
            overlay.querySelector('.btn-confirm-save').onclick = function() {{ overlay.remove(); if (onProceed) onProceed(); }};
            overlay.querySelector('.btn-confirm-cancel').onclick = function() {{ overlay.remove(); if (onCancel) onCancel(); }};
            overlay.onclick = function(e) {{ if (e.target === overlay) {{ overlay.remove(); if (onCancel) onCancel(); }} }};
        }}

        // Concatène les warnings applicables à un projet d'édition. Retourne
        // null si RAS, sinon une string HTML prête à être injectée.
        function buildEditWarnings(name, startISO, endISO, ignoreEv) {{
            var msgs = [];
            var overlaps = checkOverlap(name, startISO.substring(0,10), startISO, endISO, ignoreEv);
            if (overlaps.length) {{
                msgs.push('<strong>Chevauchement</strong> avec créneaux existants de ' + getFirstName(name) + ' : ' +
                    overlaps.map(function(o) {{ return o.start + '–' + o.end + ' ' + o.code; }}).join(', '));
            }}
            var check = check48hRolling(name, startISO, endISO);
            if (check.worstMargin !== null && check.worstMargin < 0) {{
                var w = check.worstWindow;
                var fmt = function(d) {{ return String(d.getDate()).padStart(2,'0') + '/' + String(d.getMonth()+1).padStart(2,'0'); }};
                var end = new Date(w.end); end.setDate(end.getDate() - 1);
                msgs.push('<strong>Dépassement 48h/7j</strong> sur ' + fmt(w.start) + ' → ' + fmt(end) +
                    ' : ' + formatHours(w.hours) + ' (' + formatHours(-check.worstMargin) + ' au-dessus)');
            }}
        ...
        function applyTimeEdit(empName, ev, newStart, newEnd) {{
            ...
            var warn = buildEditWarnings(empName, newStartISO, newEndISO, ev);
            if (warn) {{
                showEditWarning(warn, commit);
            }} else {{
                commit();
            }}
        }}
...
def _write_attendance_html(slug, title, sessions, groups, json_data):
    """Génère une page HTML statique de présences avec checkboxes et navigation."""
    # Build session headers
    session_headers = ""
    for s in sessions:
        cls = "vacation" if s["is_vacation"] else ""
        label = s["label"]
        session_headers += f'<th class="session-col {cls}" data-label="{label}">{label}</th>\n'
    ...
    for g_idx, group in enumerate(groups):
        ...
        for kid in group["kids"]:
            cat_span = f' <span class="name-cat">{kid["category"]}</span>' if kid["category"] else ""
            groups_html += (
                f'<tr data-row="{kid["row"]}" data-name="{kid["name"]}">\n'
                f'<td class="num-col">{kid["num"]}</td>'
                f'<td class="name-col">{kid["name"]}{cat_span}</td>'
                f'<td class="cat-col">{kid["category"]}</td>\n'
            )
            for s in sessions:
                val = kid["attendance"].get(s["label"])
                if s["is_vacation"]:
                    groups_html += f'<td class="session-cell vacation" data-label="{s["label"]}"></td>\n'
                else:
                    checked = " checked" if val == 1 else ""
                    groups_html += (
                        f'<td class="session-cell" data-label="{s["label"]}" '
            ...
            groups_html += f'<td class="total-val">{total_p}</td></tr>\n'

        # Footer total
        groups_html += '</tbody><tfoot><tr>\n'
        groups_html += '<td></td><td style="text-align:left;font-weight:700;">TOTAL</td><td></td>\n'
        for s in sessions:
            if s["is_vacation"]:
                groups_html += f'<td class="session-cell vacation" data-label="{s["label"]}"></td>\n'
            else:
                count = sum(1 for k in group["kids"] if k["attendance"].get(s["label"]) == 1)
                groups_html += f'<td class="session-cell" data-label="{s["label"]}">{count if count else ""}</td>\n'
        groups_html += '<td></td></tr></tfoot></table></div>\n'
...
<body>
    <div class="container">
        ...
        <div class="session-nav">
            <button class="session-nav-btn" id="nav-prev">&larr;</button>
            <div class="session-nav-info" id="nav-info"></div>
            <button class="session-nav-btn" id="nav-next">&rarr;</button>
        </div>
